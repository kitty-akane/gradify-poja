...
class GradeControllerIT extends AbstractIT {
  ...
  void grading_creates_the_grade_and_every_change_is_tracked_in_history() {
    ...
    // 3. correction: updates the value in place and appends a new history row
    var correction =
        restTemplate.exchange(
            path,
            PUT,
    ...
    assertThat(correction.getBody().id()).isEqualTo(gradeId); // same grade row, updated in place

    // 4. history has both changes, most recent first, with old/new values and the reason kept
    var history =
        restTemplate.exchange(
